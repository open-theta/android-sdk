...
public class VASTModel implements Serializable {
	...
	public HashMap<TRACKING_EVENTS_TYPE, List<String>> getTrackingUrls() {
		...
		try {
			...
			return new HashMap<TRACKING_EVENTS_TYPE, List<String>>(0);
		}
		...
		try {
			...
			if (nodes != null) {
				for (int i = 0; i < nodes.getLength(); i++) {
					...
					if("video/mp4".equals(mediaFile.getType()) && mediaFile.getHeight().intValue() == 360){
			...
			VASTLog.e(TAG, e.getMessage(), e);
			return new ArrayList<VASTMediaFile>(0);
		}
	...
	public String getSkipOffset() {
		...
		VASTLog.d(TAG, "getLinearAttributes");		
		XPath xpath = XPathFactory.newInstance().newXPath();
		try {
	...
	public String getPickedMediaFileURL() {
		if(pickedMediaFileURL == null){
			...
			if(mediaFiles.size()>0){
				pickedMediaFileURL = mediaFiles.get(0).getValue();	
			}
		}
